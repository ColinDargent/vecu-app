...
func synchroniseService(c serviceCfg) (installe bool, err error) {
	...
	actuel, _ := os.ReadFile(c.definition)
	...
	bak := c.definition + ".pre-vecu-app.bak"
	if len(actuel) > 0 {
	...
	if err := ecritFichierAtomique(c.definition, []byte(desire)); err != nil {
		return false, err
	}
	if err := bootstrapService(c.definition); err != nil {
		...
		if b, e := os.ReadFile(bak); e == nil && len(b) > 0 {
			if ecritFichierAtomique(c.definition, b) == nil {
				_ = bootstrapService(c.definition)
			}
		}
		return false, err
	}
...
func racineDeclaree(path string) (string, bool) {
